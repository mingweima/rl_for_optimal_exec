...
class ReplayBuffer:
    ...
    def __init__(self, buffer_size, replay_batch_size, seed):
        ...
        self.experience = namedtuple("Experience", field_names=["ob", "ac", "re", "next_ob", "done"])
        self.seed = random.seed(seed)

    def add(self, ob, ac, re, next_ob, done):
        """Add a new experience to memory."""
        e = self.experience(ob, ac, re, next_ob, done)
        self.memory.append(e)

    def sample(self):
        """Randomly sample a batch of experiences from memory."""
        experiences = random.sample(self.memory, k=1)
        obs = np.vstack([e.ob for e in experiences if e is not None])
