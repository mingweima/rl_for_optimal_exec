...
class DDDQNNet:
    def __init__(self, state_size, action_size, initial_learning_rate, name):
        ...
        with tf.variable_scope(self.name):
            ...
            # define network
            # Input is 100x120x4
            self.conv_first1 = tf.keras.layers.LSTM(24, return_sequences=True)(self.inputs_)
            ...
            self.conv_first1 = tf.keras.layers.Dense(24)(self.conv_first1)
            self.conv_first1 = tf.keras.layers.Dropout(0.2)(self.conv_first1)

            ## Here we separate into two streams
            # The one that calculate V(s)
            self.value_fc = tf.keras.layers.LSTM(24)(self.conv_first1)
            self.value_fc = tf.keras.layers.Dense(24)(self.value_fc)
            ...
            self.advantage_fc = tf.keras.layers.LSTM(24)(self.conv_first1)
            self.advantage_fc = tf.keras.layers.Dense(24)(self.advantage_fc)
